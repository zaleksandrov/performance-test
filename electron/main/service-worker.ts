...
import { convertToGPS, convertToCartesian } from "../../src/util";
...
const replyLoop = async (event: IpcMainEvent) => {
  ...
  while (true) {
    for (let i = 0; i < c.iterationCount; i++) {
      ...
      const y =
        Math.sin(((counter + iteration) % c.GRID_SIZE_X) * c.speed) * c.height;
      if (counter >= messages.length)
        messages.push(`${c.WORKER_TO_RENDERER}${counter}`);

      position.set(0, y, 0);

      const data: TelemetryData = {
        id: counter,
        position: convertToCartesian(convertToGPS(position)),
        timestamp: 0,
      };
